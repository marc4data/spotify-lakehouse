-- One row per track or episode URI seen in an export, described by the export's own metadata from its most
...
select
    content_uri,
    content_type,
    coalesce(track_name, episode_name) as content_name,
    coalesce(album_name, episode_show_name) as parent_name,
