...
| **Table alignment ships with the table, not with the page.** `display(df)` renders through `notebook.aligned_table_html`: strings (and booleans, and timestamps) left, numbers right. | `TABLE_CSS` is injected once as the setup cell's output, which is enough for the rendered report but **not for the notebook UI** — VS Code renders each cell's output in its own context, so a stylesheet from one cell need not reach another (Marc, 2026-09-17). pandas avoids this by emitting a `<style>` block with every table, so the project does the same. It declines — falling back to pandas untouched — for frames `display.max_rows` would truncate, and for non-unique columns or index, where `Styler` raises `KeyError`: a raise inside `_repr_html_` breaks the whole cell output. 🚨 **`class="dataframe"` is pinned back on**, because nb2report detects tables with a plain `'dataframe' in html` test and would otherwise lose the table wrapper and sorting silently. |
